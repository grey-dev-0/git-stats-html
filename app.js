...
let dom = new JSDOM('<!DOCTYPE html><html lang="en"><head><title>Git Report</title>'+
    '<style>#grid .title{height:16px;font-size:12px}#grid #hcol{display:inline-block;vertical-align:top}#grid #hcol .title:first-child{margin-bottom:6px}#grid #hcol .title{margin-right:6px}#grid .col{display:inline-block;width:16px}#grid .col .title{margin-bottom:6px}#grid .col .day{width:16px;height:16px}</style>'+
    '</head><body></body></html>');
$ = $(dom.window);
...
function render(data){
    ...
    let container = $('<div id="container"><div id="grid"><div id="hcol"><div class="title"></div><div class="title">'+
        ...
        +commits.length+'</div><div class="info"><strong>Current Streak</strong><br/>'+currentStreakCount+' Days, from '
        +moment(currentStreakPeriod.start).format('YYYY-MM-DD')+' to '+moment(currentStreakPeriod.end).format('YYYY-MM-DD')
        +'</div><div class="info"><strong>Longest Streak</strong><br/>'+longestStreakCount+' Days, from '
